...
function createProducts(product) {
  return `
    ...
    <div class="card-content">
      <p>${product.rating.rate} ⭐ | ${product.rating.count}k</p>
      <h2>${product.title}</h2>
...
function sortByPrice() {
  ...
  if (val === "choose") {
    ...
    let sortedProducts = desPriceSorting(updatedProducts);
    mapProducts(sortedProducts);
  } else if (val === "asc") {
    let sortedProducts = ascPriceSorting(products);
    mapProducts(sortedProducts);
  } else if (val == "des") {
    let sortedProducts = desPriceSorting(products);
    mapProducts(sortedProducts);
  }
}
// Event Listener
...
// Sort by popularity functionality implemented
function ascPopularSorting(products) {
...
function desPopularSorting(products) {
  const sortedProducts = products.toSorted(
...
popular.addEventListener("change", sortByPopularity);
